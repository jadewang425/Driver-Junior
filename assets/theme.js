...
updateFacets_fn = function() {
  ...
  // Driver - All filter menu accordions open by default, each time the menu is reopened
  Array.from(this.querySelectorAll("details")).forEach((detail) => detail.open = true);
};
...
var Listbox = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    /* Driver Select-A-Size -- enable selection of default variant within variant-picker dropdown used in product-card-quick-buy  
    this.quickBuyElements = document.querySelectorAll('quick-buy');*/
  }

  /* Driver Select-A-Size -- enable selection of default variant within x-listbox / dropdown 
  driverSelectDefaultVariantDropdown() {
    this.quickBuyElements.forEach(quickBuy => {
      let quickBuyForm = quickBuy.getAttribute('form');
      let buyButtons = quickBuy.querySelector('.buy-buttons');
    
      if (buyButtons && buyButtons.getAttribute('form') === quickBuyForm) {
        let buttons = buyButtons.querySelectorAll('button');
        let controlButton = quickBuy.querySelector('variant-picker button[aria-controls]');
        let popoverId = controlButton ? controlButton.getAttribute('aria-controls') : null;
        let popover = popoverId ? document.getElementById(popoverId) : null;
        let option = popover ? popover.querySelector('x-listbox button') : null;
        // console.log('option', option);

        buttons.forEach(button => {
          // Enable ATC button on variant selection
          if (option && __privateGet(this, _hiddenInput).value === option.getAttribute('value')) {
...
onOptionClicked_fn = function(event) {
  ...
  /* Driver Select-A-Size -- enable selection of default variant within x-listbox / dropdown 
  this.driverSelectDefaultVariantDropdown(); */
...
var ProductList = class extends HTMLElement {
  connectedCallback() {
    ...
    // DRIVER Sort Collections by Availability
    document.addEventListener('DOMContentLoaded', () => this.sortProducts());

    // Listen for the Driver collection:updated event and re-sort the products
    // This event fires in facet-listeners.js and load-more.js
    document.addEventListener('collection:updated', () => this.sortProducts());
  ...
  // DRIVER Sort Collections by Availability
  sortProducts() {
    const productCards = Array.from(this.querySelectorAll('product-card'));
    const available = productCards.filter(card => card.classList.contains('available'));
    const bis = productCards.filter(card => card.classList.contains('bis'));
    const oos = productCards.filter(card => card.classList.contains('oos'));

    const sortedProducts = [...available, ...bis, ...oos];

    sortedProducts.forEach(product => this.appendChild(product));
  }
};
...
var VariantPicker = class extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    /* Driver Select-A-Size -- enable selection of default variant  */
    this.buttonSelectors = document.querySelectorAll('.buy-buttons');
  ...
  /* Driver Select-A-Size -- enable selection of default variant within block variant-pickers */
  driverSelectDefaultVariant() {
    this.buttonSelectors.forEach(button => {
      let buttonForm = button.getAttribute('form');
      let form = document.querySelector(`#${buttonForm}`);

      // Enable ATC button on variant selection
      if (form && form.id.value == this.masterSelector.value) {
          button.querySelector('button').disabled = false;
          button.querySelector('button').textContent = 'Add To Bag';
      }    
    });
  }
...
onOptionChanged_fn = function(event) {
  if (event.target.name.startsWith("option")) {
    /* Driver Select-A-Size within block-style variant-picker */
    this.driverSelectDefaultVariant();
  }
  if (!event.target.name.startsWith("option")) {
    return;
  }
...
if(optionSelector.querySelector('input[name^="option"][type="hidden"], input[name^="option"]:checked, select[name^="option"]') != null){
...
onSidebarAfterHide_fn = function() {
  ...
  // Driver - All mobile menu accordions open by default, each time the menu is reopened
  Array.from(this.querySelectorAll("details")).forEach((detail) => detail.open = true);
};
...
loadRecommendations_fn = async function() {
  ...
  if (productRecommendationsElement.childElementCount > 0) {
    ...
    this.hidden = false;
    /* Driver - Swym */
    console.log('init Swym')
    _swat.initializeActionButtons('main');
  } else {
    this.remove();
  }
};
